        ...
        <div>
            <section className="text-gray-600 body-font min-h-screen">
                <div className=" px-5 py-10 mx-auto">
                    ...
                    {Object.keys(products).length === 0 && <p className='text-xl font-bold'>Sorry All MousePads are currently out of stock. New stock coming soon. Stay Tuned!</p>}
...
export async function getServerSideProps(context) {
    ...
    for (let item of products) {
        if (item.title in mousepads) {
            if (!mousepads[item.title].color.includes(item.color) && item.availaibleQty > 0) {
                mousepads[item.title].color.push(item.color);
            }
            if (!mousepads[item.title].size.includes(item.size) && item.availaibleQty > 0) {
            ...
            if (item.availaibleQty > 0) {
                ...
                mousepads[item.title].color = [];
                mousepads[item.title].size = [];
            }
        }
    }
    return {
        props: { products: JSON.parse(JSON.stringify(mousepads)) }
    }
}
