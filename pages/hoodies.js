      ...
      <section className="text-gray-600 body-font min-h-screen">
        <div className=" px-5 py-10 mx-auto">
          <div className="flex flex-wrap justify-center">
            ...
            {Object.keys(products).length === 0 && <p className='text-xl font-bold'>Sorry All Hoodies are currently out of stock. New stock coming soon. Stay Tuned!</p>}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in hoods) {
      if (!hoods[item.title].color.includes(item.color) && item.availaibleQty > 0) {
        hoods[item.title].color.push(item.color);
      }
      if (!hoods[item.title].size.includes(item.size) && item.availaibleQty > 0) {
