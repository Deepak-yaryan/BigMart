...
const Post = ({ setProgress, addToCart, product, variants, buyNow, error }) => {
  ...
  let refreshVariant = (newSize, newColor) => {
            ...
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
              ...
              <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                <div className="flex">
                  ...
                  {Object.keys(variants).includes('White') && Object.keys(variants['White']).includes(size) && <button onClick={() => { refreshVariant(size, 'White') }} className={`border-2  ${(color === 'White') ? ('border-black') : ('border-gray-300')} ml-1 bg-white rounded-full w-6 h-6 focus:outline-none`}></button>}
                ...
                <div className="flex ml-6 items-center">
                  ...
                  <div className="relative">
                    ...
                    <button className="rounded-full w-10 h-10 bg-gray-200 p-0 border-0 inline-flex items-center justify-center text-gray-500 ml-20">
                  <svg fill="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" className="w-5 h-5" viewBox="0 0 24 24">
              ...
              <div className='pin mt-2 flex space-x-2 text-sm'>
                <input onChange={onChangePin} id='checkin' className='px-2 border-2 border-gray-400 rounded-md' type="text" placeholder='Enter Pincode' />
                <button onClick={checkServiceability} id='check' className="flex ml-14 text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Check</button>
              </div>
              {(!service && service != null) && <div className='text-red-700 text-sm mt-3 font-semibold'>
                No seller ship to this pincode yet
              </div>}
              {(service && service != null) && <div className='text-green-700 text-sm mt-3 font-semibold'>
...
export async function getServerSideProps(context) {
  ...
  return {
    props: {error: error, variants: JSON.parse(JSON.stringify(ColorSizeSlug)), product: JSON.parse(JSON.stringify(product)) }
  }
}
